...
def run_game():
    ...
    while True:
        gf.check_events(ai_settings,screen,stats,play_button,ship,aliens,bullets)#按键等事件检查
        if stats.game_active:
            ship.update()#不断更新飞船的位置坐标
            gf.update_bullets(ai_settings,screen,ship,aliens,bullets)#不断更新子弹坐标，已经将不在屏幕内的子弹从组内删除
            gf.update_aliens(ai_settings,stats,screen,ship,aliens,bullets)
        gf.update_screen(ai_settings,screen,stats,ship,aliens,bullets,play_button)#不断重绘屏幕，飞船，子弹
